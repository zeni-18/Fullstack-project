            ...
            >
                <div className="flex" style={{ gap: '16px', alignItems: 'center' }}>
                    <Link to={`/profile/${user.username}`}>
                        <img
                            src={user.profileImage ? user.profileImage : `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username}`}
                        ...
                        >
                            <div className="flex" style={{ gap: '12px', alignItems: 'center' }}>
                                <Link to={`/profile/${sug.username}`}>
                                    <img
                                        src={sug.profileImage ? sug.profileImage : `https://api.dicebear.com/7.x/avataaars/svg?seed=${sug.username}`}
                                        style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', border: '1px solid var(--border)' }}
                                        alt="suggestion"
                                        onError={(e) => {
                                            e.target.src = `https://api.dicebear.com/7.x/avataaars/svg?seed=${sug.username}`;
                                        }}
                                    />
                                </Link>
                                <div>
